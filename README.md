...
re-verified, and one confirmed manuscript error (Table 7's Scheme D rho)
caught by cross-checking against an independent Python recomputation.
Each `.do` file's header documents its own verification status, and each
ends with an "EXPECTED OUTPUT" block for anyone who wants to re-check it
themselves.
